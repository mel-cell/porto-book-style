  ...
  return (
    <div className="relative flex flex-col items-center gap-6">
      ...
      >
        ...
        {/* 3D Book Rotation Wrapper */}
        <motion.div
           animate={{ rotateY: isCVOpen ? -20 : 0 }}
           transition={{ type: "spring", damping: 24, stiffness: 100 }}
            ...
            <div className="flex-1 h-full relative bg-[var(--bg-page-alt)] perspective-[1500px] perspective-origin-[100%_50%] rounded-l-[2px] shadow-[inset_-12px_0_20px_rgba(0,0,0,0.06)]">
              ...
              {isBackward && (
                <div ref={flipCardRef} className="absolute inset-0 w-full h-full preserve-3d will-change-transform z-[20] origin-[100%_80%] !transition-none">
                  <div className="absolute inset-0 backface-hidden overflow-hidden bg-[var(--bg-page)]">{get(leftIdx)}</div>
                  <div className="absolute inset-0 backface-hidden overflow-hidden bg-[var(--bg-page-alt)]" style={{ transform: 'rotateY(-180deg) scaleX(-1)' }}>{get(underBackwardRight)}</div>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 h-full relative bg-[var(--bg-page)] perspective-[1500px] perspective-origin-[0%_50%] rounded-r-[2px] shadow-[inset_12px_0_20px_rgba(0,0,0,0.06)]">
              ...
              {isForward && (
                <div ref={flipCardRef} className="absolute inset-0 w-full h-full preserve-3d will-change-transform z-[20] origin-[0%_80%] !transition-none">
                  <div className="absolute inset-0 backface-hidden overflow-hidden bg-[var(--bg-page)]">{get(rightIdx)}</div>
                  <div className="absolute inset-0 backface-hidden overflow-hidden bg-[var(--bg-page-alt)]" style={{ transform: 'rotateY(180deg) scaleX(-1)' }}>{get(underForwardLeft)}</div>
                </div>
              )}
            </div>
          </div>
        ...
        {/* HUD */}
        <div className="flex items-center justify-between w-full max-w-[calc(var(--book-width)*2)] z-[10] px-2 mt-4">
          <span className="text-[10px] font-mono text-[var(--text-muted)] opacity-70 w-1/3">{showPrev ? "← drag" : ""}</span>
          <span className="text-[11px] font-mono text-[var(--text-secondary)] bg-black/[0.04] border border-[var(--border)] px-4 py-1 rounded-full">{spreadNum} / {totalSpreads}</span>
          <span className="text-[10px] font-mono text-[var(--text-muted)] opacity-70 w-1/3 text-right">{showNext ? "drag →" : ""}</span>
        </div>

        {/* Action Buttons */}
        <div className="absolute -top-10 right-0 flex items-center gap-3 z-[100]">
          <motion.button
            className={`flex items-center gap-2 h-7 px-4 rounded-full border transition-all shadow-sm ${isCVOpen ? 'bg-black text-white border-black' : 'bg-black/[0.04] border-[var(--border-strong)] text-[var(--text-muted)]'}`}
          ...
          <motion.button
            className={`w-7 h-7 rounded-full border flex items-center justify-center transition-all shadow-sm ${isDrawing ? 'bg-black text-white border-black' : 'bg-black/[0.04] border-[var(--border-strong)] text-[var(--text-muted)]'}`}
            onClick={() => setIsDrawing(!isDrawing)}
            whileHover={{ scale: 1.1 }}
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-[14px] h-[14px]"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
          </motion.button>

          <motion.button
            className="w-7 h-7 rounded-full border border-[var(--border-strong)] bg-black/[0.04] text-[var(--text-muted)] flex items-center justify-center hover:bg-red-50 hover:text-red-500"
            onClick={onClose}
            whileHover={{ scale: 1.1 }}
          >
            ✕
